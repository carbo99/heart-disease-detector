...
somma_pesata( Esempi, Att, [Val|Valori], SommaParziale, Somma) :-
	...
	findall(P,			          
                ...
                ClDst),
        entropia(ClDst,Ent),
	NuovaSommaParziale is SommaParziale + Ent*NVal/N,
...
entropia(ListaProb, Entropia) :-
	entropia(ListaProb, 0, Entropia).

entropia([], Acc, Acc).
entropia([P|Ps], Acc, Entropia) :-
	P > 0, !,
	Acc1 is Acc - P * log(P) / log(2),
	entropia(Ps, Acc1, Entropia).
entropia([_|Ps], Acc, Entropia) :-
	entropia(Ps, Acc, Entropia).
